...
gulp.task('revision', function(done) {
  ...
  var files = [path.join(__dirname, 'views/pages/index.ejs'), path.join(__dirname, 'gulpfile.js')]
    ...
    .map(function(asset) {
      switch (path.extname(asset)) {
        case '.jpg':
          return {
            type: 'image',
            url: 'https://s3.amazonaws.com/tc3-whitecube/rosenblit/' + path.basename(asset, '.jpg'),
          }
        case '.ogg':
          return {
            type: 'audio',
            url: 'https://s3.amazonaws.com/tc3-whitecube/rosenblit/' + path.basename(asset, '.ogg'),
          }
        case '.webm':
          return {
            type: 'video',
            url: 'https://s3.amazonaws.com/tc3-whitecube/rosenblit/' + path.basename(asset, '.webm'),
          }
        default:
          return null
      }
